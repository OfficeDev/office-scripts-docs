...
Office Scripts is currently available for Excel on Windows, Mac, and the web. The full Power Query experience is currently [only available for Excel on Windows](/power-query/power-query-what-is-power-query#where-can-you-use-power-query). Both can be used through Power Automate, which lets the flow work with Excel workbooks stored in OneDrive.
