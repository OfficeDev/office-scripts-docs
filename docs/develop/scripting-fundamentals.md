...
Ranges use two-dimensional arrays to manage information. Read the [Working with ranges section](javascript-objects.md) of [Built-in JavaScript objects in Office Scripts](javascript-objects.md#working-with-ranges) for more information on handling those arrays in the Office Scripts framework.
...
The returned array is a normal array, so all the regular array operations are available for your script. You can also access individual objects within the collection using the array index value. For example, `workbook.getTables()[0]` returns the first table in the collection. Read the [Working with collections section](javascript-objects.md#working-with-collections) of (Built-in JavaScript objects in Office Scripts](javascript-objects.md) to learn more about using built-in array functionality with the Office Scripts framework.
